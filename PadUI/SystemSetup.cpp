...
{
    LCD_TP_RESET_Pin = reset;
    LCD_RESET_Pin    = reset;
    QSPI_RESET_Pin   = reset;
    WIFI_RESET_Pin   = reset;
}
...
extern "C" void PreBSSInitSetup()
{
    ...
    LCD_TP_RESET_Pin.SetDirection(DigitalPinDirection_e::Out);
    LCD_RESET_Pin.SetDirection(DigitalPinDirection_e::Out);
    QSPI_RESET_Pin.SetDirection(DigitalPinDirection_e::Out);
    WIFI_RESET_Pin.SetDirection(DigitalPinDirection_e::Out);

    SDRAM_BANK0_Pin.SetDirection(DigitalPinDirection_e::Out);
    SDRAM_BANK1_Pin.SetDirection(DigitalPinDirection_e::Out);
    SDRAM_CKE_Pin.SetDirection(DigitalPinDirection_e::Out);
    SDRAM_CLK_Pin.SetDirection(DigitalPinDirection_e::Out);

    SetExternalReset(false);

    kernel::Kernel::PreBSSInitialize(CLOCK_CRYSTAL_FREQUENCY, CLOCK_CPU_FREQUENCY, CLOCK_PERIF_FREQUENCY);
    ...
    SetupSDRAM(SDRAMC_CR_NC_COL10     |
               ...
               25,
               7812,
               CLOCK_PERIF_FREQUENCY);
    SCB_CleanInvalidateDCache();    
}

extern "C" void PostBSSInitSetup()
{
    SAME70System::SetupFrequencies(CLOCK_CRYSTAL_FREQUENCY, CLOCK_CPU_FREQUENCY, CLOCK_PERIF_FREQUENCY);
...
//    dw_region_attr = MPU_AP_FULL_ACCESS | INNER_NORMAL_WB_RWA_TYPE( SHAREABLE ) | mpu_cal_mpu_region_size(SRAM_FIRST_END_ADDRESS - SRAM_FIRST_START_ADDRESS) | MPU_REGION_ENABLE;
...
//    dw_region_attr = MPU_AP_FULL_ACCESS | INNER_NORMAL_WB_RWA_TYPE( SHAREABLE ) | mpu_cal_mpu_region_size(SRAM_SECOND_END_ADDRESS - SRAM_SECOND_START_ADDRESS) | MPU_REGION_ENABLE;
